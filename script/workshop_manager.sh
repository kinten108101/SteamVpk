...
workshop_download ()
{
    ...
    if [ "${fileId}" == "" ]; then
    {
        ulog "You have not entered ID of the published workshop item!" 'E'
        exit 1
    }
    fi
    vpkname="${fileId}"
...
{
    cat <<END
usage: workshop_manager <command>
commands:
    -h, --help                  Help menu
    -s, --source                Database of published item.
                              Currently supports 'STEAMWORKSHOP'
                              To be added is 'GAMEMAPS'.
    -i, --id                    ID of published item. Required.
    -n, --name                  Name of published item. Optional.
    -f, --file                  Select to download published item's file.
    -m, --image                 Select to download published item's thumbnail.
    -fm,--full                  Select to download both of the above.
    -x, --info-stdout           Select to print published item's attributes
                              (title, author, categories, description, ...)
                              to standard output.

END
    return 0
    # by default, zero is returned!
}
...
eval set -- "${args}"
while getopts ':hs:i:n:ofmx' OPTION; do
    case ${OPTION} in
...
if [ "${DOWNLOAD_FILEID}" != "${UNSET_CODE}" ] && [ "${DOWNLOAD_SOURCE}" != "${UNSET_CODE}" ]; then
    case ${DOWNLOAD_SOURCE} in
        (STEAMWORKSHOP) workshop_download "$( read_mylogin ),${DOWNLOAD_FILEID}" && exit 0;;
        (GAMEMAPS) ulog 'Stay tuned!' 'E' && exit 1;;
